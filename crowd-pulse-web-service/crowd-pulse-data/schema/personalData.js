...
PersonalDataSchema.statics.statDiagnosisBarTypeDataTelegram = function (from, to) {
  var Q = this.aggregate(buildDiagnosisTypeFilterQueryTelegramBar(from, to)).exec();

  return Q
};

PersonalDataSchema.statics.statAnalysisLineTypeDataTelegram = function (from, to, name) {
  var Q = this.aggregate(buildAnalysisTypeFilterQueryTelegramLine(from, to, name)).exec();

  return Q
};

PersonalDataSchema.statics.statTherapyTableTypeDataTelegram = function (from, to) {
  var Q = this.aggregate(buildTherapyTypeFilterQueryTelegramTable(from, to)).exec();
...
var buildDiagnosisTypeFilterQueryTelegramBar = function (from, to) {
  var filter = undefined;
  ...
  if (hasFrom || hasTo) {
    ...
    if (hasFrom || hasTo) {
      ...
      if (hasFrom) {
        ...
        filter.$match['timestamp']['$gte'] = Math.floor(from.getTime() / 1000);
        console.log("dopo di "+ filter.$match['timestamp']['$gte']); 
        
      }
      if (hasTo) {
        
        filter.$match['timestamp']['$lte'] = Math.floor(to.getTime() / 1000);
        console.log("prima di "+  filter.$match['timestamp']['$lte'] ) 
...
var buildAnalysisTypeFilterQueryTelegramLine = function (from, to, name) {
  var filter = undefined;
...
var buildTherapyTypeFilterQueryTelegramTable = function (from, to) {
  var filter = undefined;
